...
const Products = () => {
    ...
    const { data: response, isLoading, error } = useQuery({
        ...
        queryFn: async () => {
            const res = await fetch(`http://localhost:3000/api/products?search=${debouncedSearch}&page=${page}&limit=20`);
                                ...
                                <TableRow key={product.id}>
                                    <TableCell>
                                        {product.foto_produto ? (
                                            <img src={`http://localhost:3000/uploads/${product.foto_produto}`} alt="" className="w-12 h-12 object-cover rounded" />
                                        ) : (
